<?php
class agregatipodocumentoModel extends Model{
...
public function guardatipodocumento($descripcion,$abreviatura){
    $descripcion = $_POST['txt_desc'];
    $abreviatura = $_POST['txt_abre'];
    $respuesta = array();//para obtener todos los datos defines un vector o un arreglo
    $conn = $this->db->connect();
    $query = $conn->prepare("insert into tipo_documento (descripcion,abreviatura) values ('$descripcion','$abreviatura')");
    //$dtype = "ss";
    //$query->bind_param($dtype,$data['usr'], $data['pss']);
    $query->execute();
    //$query->bind_result($rs);
