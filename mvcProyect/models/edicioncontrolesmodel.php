<?php
class edicioncontrolesModel extends Model{
...
public function guardacontrol($id_control,$descripcion){
    $id_control = $_POST['id_control'];
    $descripcion = $_POST['txt_desc']; 
    $respuesta = array();//para obtener todos los datos defines un vector o un arreglo
    $conn = $this->db->connect();
    $query = $conn->prepare("update controles set descripcion='$descripcion' where id_control='$id_control'");
    //$dtype = "ss";
    //$query->bind_param($dtype,$data['usr'], $data['pss']);
    $query->execute();
    //$query->bind_result($rs);
